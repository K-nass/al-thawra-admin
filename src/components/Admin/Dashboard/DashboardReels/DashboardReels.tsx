...
export default function DashboardReels() {
  ...
  const { data, isLoading, isError, fetchNextPage, hasNextPage, isFetchingNextPage } = useFetchReelsFeed(10);

  const [viewingVideoUrl, setViewingVideoUrl] = useState<string | null>(null);
  const [confirmDelete, setConfirmDelete] = useState<{ isOpen: boolean; reelId: string | null; reelCaption: string }>({
    isOpen: false,
    reelId: null,
    reelCaption: "",
  });
  ...
  const handleDeleteClick = (reel: Reel) => {
    setConfirmDelete({
      isOpen: true,
      reelId: reel.id,
      reelCaption: reel.caption || t("reels.thisReel"),
    });
  };

  const handleConfirmDelete = async () => {
    if (!confirmDelete.reelId) return;
    try {
    ...
    <div className="flex-1 flex flex-col min-h-0 bg-surface">
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
          <div>
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">{t("dashboard.reels")}</h1>
            <p className="text-sm text-slate-500 mt-1">{t("reels.subtitle")}</p>
          </div>
          <a
        ...
        {isLoading && reels.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center bg-white rounded-xl border border-slate-200 shadow-sm">
            <Loader />
            <p className="mt-4 text-sm text-slate-500 animate-pulse">{t("reels.loadingReels")}</p>
          </div>
        ) : isError ? (
          <div className="p-8 bg-error-background/50 border border-error-border rounded-xl text-center">
            ...
            <h3 className="text-lg font-semibold text-error-hover mb-2">{t("error.failedToLoadReels")}</h3>
          </div>
        ) : reels.length === 0 ? (
          <div className="py-20 flex flex-col items-center justify-center bg-white rounded-xl border border-slate-200 shadow-sm">
            ...
            <p className="text-slate-500 font-medium">{t("common.noRecordsFound")}</p>
          </div>
        ) : (
              ...
              <TableBody>
                {reels.map((reel: Reel) => (
                  <TableRow key={reel.id}>
                    <TableCell className="text-xs font-mono text-slate-400">#{reel.id.slice(-4)}</TableCell>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        {reel.thumbnailUrl ? (
                          <div className="relative group flex-shrink-0">
                            <img src={reel.thumbnailUrl} alt={reel.caption || t("reels.untitled")} className="w-12 h-16 object-cover rounded-lg shadow-sm bg-slate-100" />
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="font-semibold text-slate-900 truncate max-w-[200px]">{reel.caption || t("reels.untitled")}</span>
                          <span className="text-xs text-slate-500 flex items-center gap-1">
                            <MessageSquare size={10} />
                            {reel.duration || t("reels.defaultDuration")}
                          </span>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium text-slate-700">{reel.userName || t("reels.defaultAuthor")}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <Badge variant={reel.isPublished ? "success" : "warning"}>{reel.isPublished ? t("post.published") : t("post.draft")}</Badge>
                    </TableCell>
                    <TableCell>
                ...
                <tr ref={observerTarget}>
                  <td colSpan={8} className="py-8 text-center">
                    {isFetchingNextPage ? (
                      ...
                      <div className="h-1 text-transparent">{t("common.scrollTrigger")}</div>
                    ) : reels.length > 0 ? (
        ...
        {viewingVideoUrl && (
          <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4">
            ...
            >
              <button
                onClick={() => setViewingVideoUrl(null)}
                className="absolute top-4 right-4 z-20 bg-black/40 text-white hover:bg-black/60 transition-colors p-2 rounded-full backdrop-blur-sm"
              >
                <X className="w-5 h-5" />
              </button>
              <video src={viewingVideoUrl} controls autoPlay className="w-full h-full" />
            </motion.div>
          </div>
        )}
      </AnimatePresence>
      ...
      <ConfirmDialog
        ...
        message={t("reels.deleteMessage", { title: confirmDelete.reelCaption })}
