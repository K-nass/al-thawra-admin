   ...
   <div className="w-full flex flex-col gap-6 p-2 bg-white">
      ...
      <div className="flex justify-center">
        <div className="grid grid-cols-2 gap-6 w-full max-w-5xl">
          ...
          {/* Left Side: Text Only with Hover Hint */}
      ...
      <div className="grid grid-cols-4 border-t-2 border-dashed border-gray-400 pt-6">
        {[1, 2, 3, 4].map((i) => (
          <div 
            key={i} 
            className={`p-3 h-24 flex flex-col justify-between ${i < 4 ? 'border-r-2 border-dashed border-gray-400' : ''}`}
          >
            <div className="h-3 w-full bg-gray-200 rounded" />
            <div className="h-2 w-2/3 bg-gray-100 rounded mt-auto" />
          </div>
        ))}
      </div>
    </div>
